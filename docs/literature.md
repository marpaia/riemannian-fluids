...
Formal implementation: a canonical adjoint-based orthogonal splitting for isometric immersions; tangent and normal covariant derivatives constructed by extending, differentiating, and projecting; proved Gauss--Weingarten decompositions; a continuous bilinear kernel-normal second fundamental form and continuous trilinear `∇ᴮ II`; a general theorem that Lie brackets preserve `f`-relatedness for arbitrary smooth maps between boundaryless manifolds; an induced Levi--Civita connection proved metric-compatible, torsion-free, and independent of compatible extension choices on differentiable germs; germ-local ambient Levi--Civita restriction locality; symmetry of `II` and `∇ᴮ II`; source-sign-correct uncontracted and contracted Codazzi; shape operators and mean curvature by Riesz duality and trace; bracket--Weingarten derived from a first-order mean-curvature-field realization; vector, scalar, and flat-ambient Euclidean Gauss equations on actual fibers; a paper-facing codimension-two Ricci contraction on `Fin 2` frames; an adapted ambient basis with proved Ricci trace splitting; and actual-fiber Bochner and Hodge Gauss formulas from one actual `C²` source tangent field and chosen `C²` ambient extension. The intrinsic Bochner value is now the immersion differential of the constructed rough Laplacian, the constructed ambient rough operator is expanded in the adapted frame, and canonical normal extensions identify its complete normal Hessian trace. Thus the only remaining ambient Bochner identification is the explicit varying-field tangent-Hessian trace theorem. Under that theorem and pointwise Ricci symmetry, the paper-facing values are the independently constructed Hodge Laplacians as well. Exact reproduction still requires deriving the compatible extension package, differentiated regularity, mean-curvature first jet, tangent-Hessian theorem, and Levi--Civita Ricci symmetry from the smooth geometry rather than supplying them.
