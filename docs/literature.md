...
Formal implementation: a canonical adjoint-based orthogonal splitting for isometric immersions; tangent and normal covariant derivatives constructed by extending, differentiating, and projecting; proved Gauss--Weingarten decompositions; a continuous bilinear kernel-normal second fundamental form with canonical `C²` source extensions; a general theorem that Lie brackets preserve `f`-relatedness for arbitrary smooth maps between boundaryless manifolds; an induced Levi--Civita connection proved metric-compatible, torsion-free, and independent of the compatible extension choice on differentiable germs; a metric-compatibility proof of ambient Levi--Civita restriction locality along immersed tangent directions; symmetry derived internally; shape operators and mean curvature constructed by Riesz duality and trace; differentiated field-level Gauss with field/pointwise shape agreement; vector, scalar, and flat-ambient Euclidean Gauss equations on actual fibers; a paper-facing codimension-two Ricci contraction on `Fin 2` tangent and normal frames; an adapted ambient orthonormal basis with a proved connection-derived Ricci trace splitting; and arbitrary-codimension Bochner and Hodge trace theorems. The remaining gates are realization of the differentiated-extension contract by a smooth tubular extension and the complete twice-differentiated jet with Codazzi and bracket--Weingarten.
