...
Formal implementation: a canonical adjoint-based orthogonal splitting for isometric immersions; tangent and normal covariant derivatives constructed by extending, differentiating, and projecting; proved Gauss--Weingarten decompositions; a continuous bilinear kernel-normal second fundamental form with canonical-extension regularity; an induced Levi--Civita connection proved metric-compatible, torsion-free, and independent of the compatible extension choice on differentiable germs; symmetry derived from ambient torsion-freeness and full `f`-related bracket compatibility; shape operators and mean curvature constructed by Riesz duality and trace; arbitrary-codimension Bochner and Hodge trace theorems; and a same-source package containing induced curvature, `II`, and tangentially projected ambient connection curvature on actual Mathlib fibers. The remaining gate is to prove `f`-related bracket naturality and connection restriction locality, then derive vector/scalar Gauss, Codazzi, the twice-differentiated jets, and Ricci trace splitting from the connections.
