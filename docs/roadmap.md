...
| M2: geometric operator kernel | the CZ24, CCD17, CCY23, CCG25, CCF25, and WBK26 operator identities and constructions are proved at their exact source scope | active; CCG25 now reaches constructed rough/Hodge operators behind one exact tangent-Hessian identity and standard Ricci symmetry, while the common tubular field realization remains open |
...
| `CCG25-bochner-laplacian-gauss-general-codimension` | the intrinsic value is the constructed rough Laplacian; the ambient constructed rough operator is split in an adapted frame; the complete normal trace is identified; and one explicit tangent-Hessian trace premise yields both actual-operator Gauss formulas | construct the compatible extension package, differentiated regularity, and mean-curvature first jet from one tubular neighborhood, then derive the varying-field tangent-Hessian trace premise from that package |
| `CCG25-hodge-laplacian-gauss-general-codimension` | scalar Gauss, adapted Ricci splitting, and both Hodge formulas are proved on the same actual fibers; the jet values equal the constructed Hodge operators under the common tangent-Hessian trace and intrinsic/ambient Ricci symmetry | discharge the Bochner tubular/tangent-trace gate and prove Ricci symmetry for the induced and ambient Levi--Civita connections from reusable curvature symmetries |
...
The critical path is therefore no longer formula discovery, tensoriality of `∇ᴮ II`, Codazzi, bracket--Weingarten, vector/scalar Gauss, ambient Ricci splitting, construction of the source field's intrinsic and ambient Hessians, realization of the raw normal iteration, or a generic operator-identification problem. It has two reusable geometric obligations. First, construct one smooth-neighborhood field package from tubular retraction/transport data; for CCG25 it must supply compatible ambient extensions, differentiated regularity, the mean-curvature normal-field first jet, and the actual varying-field tangent-Hessian trace theorem, while for CCY23 it must supply joint angular-radial smoothness and the coordinate jet already consumed by the ellipsoid result. Second, prove the standard Levi--Civita curvature symmetries strongly enough to derive intrinsic and ambient Ricci symmetry for the constructed Hodge bridge. Once these are stable, the CCG25 source signatures can be inhabited without caller-supplied geometric facts, the CCY23 restriction and expansion can use the same neighborhood mechanism, and the remaining WBK26 flow-rate adapter is bounded cleanup. Only then do the six remaining fragments qualify for promotion and M2 close.
