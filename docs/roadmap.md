...
The algebraic trace kernel and the first source-setting bridge are complete. The bridge now constructs the canonical orthogonal splitting of an isometric immersion by `df†`, constructs tangent and normal derivatives by extending, differentiating, and projecting, proves the Gauss--Weingarten decompositions, packages the projected derivative as a continuous bilinear map into the actual kernel-normal fiber, and derives `C²` regularity of its canonical source extensions. The tangential projection is a bundled covariant derivative; for an isometric immersion into an ambient Levi--Civita manifold it is proved metric-compatible and torsion-free, hence supplies a genuine induced Levi--Civita connection. Lie brackets of `f`-related fields are proved natural for arbitrary smooth maps between boundaryless manifolds, without an inverse-derivative assumption, so bracket compatibility and second-fundamental-form symmetry are derived internally. A general Levi--Civita uniqueness theorem proves that the induced derivative is independent, on differentiable germs, of the compatible extension operator used to construct it. A second metric-compatibility theorem proves the needed restriction locality: ambient fields agreeing along an immersion have equal Levi--Civita derivatives in immersed tangent directions. Under an explicit regularity-only contract for the ambient extension operator, Lean now differentiates the constructed Gauss splitting, proves the field-level shape adjunction, identifies it with the pointwise Riesz shape operator, and derives the vector and scalar Gauss equations from the same induced/ambient connection package. Flat ambient curvature then yields a paper-facing codimension-two Ricci theorem on actual `Fin 2` tangent and normal frames. The ambient Ricci bridge is also constructed rather than assumed: the intrinsic and kernel-normal orthonormal frames are combined into an adapted ambient basis, the normal-frame Ricci form is raised by Riesz duality, and the full ambient trace is proved to split into tangent-frame, normal-frame tangent, and normal-output terms. CCY23 equation (4.6) is likewise a construction: its surface trace, positive-radius smoothness, weighted ODE, and exact divergence cancellation are proved on every angular ray. These are deliberately recorded as proof progress rather than as closed source claims while the tubular regularity realization, complete differentiated jet, and joint-smoothness bridges remain open.
...
| `CCG25-bochner-laplacian-gauss-general-codimension` | actual extend--differentiate--project tangent and normal derivatives satisfy Gauss and Weingarten; both arbitrary-codimension trace formulas are proved from the resulting differentiated-jet shape | construct the complete twice-differentiated jet and derive contracted Codazzi and bracket--Weingarten from the immersion connection |
| `CCG25-hodge-laplacian-gauss-general-codimension` | the Ricci trace and both Hodge formulas are proved; an adapted ambient orthonormal basis now derives the ambient Ricci trace splitting for the actual connection-constructed jet | connect that constructed Ricci jet to the completed twice-differentiated Gauss--Weingarten jet |
...
The critical path is therefore no longer formula discovery, first derivatives, bracket naturality, canonical source-extension regularity, extension-choice independence, Levi--Civita restriction locality, vector/scalar Gauss, or ambient Ricci trace splitting. It is now to realize the differentiated-extension contract from a smooth tubular operator, construct the full twice-differentiated Gauss--Weingarten jet, and derive Codazzi and bracket--Weingarten. In parallel, a parametric integration theorem must upgrade CCY23's proved raywise coefficient to a jointly smooth angular-radial field and feed its derivatives into the existing ellipsoid jet. Those two routes meet at the smooth neighborhood-field adapter. The remaining intrinsic flow adapter for WBK26 is bounded cleanup once that geometric layer is stable. Only after all six fragments cross those gates does the ledger promote them to `formally-reproduced` and M2 close.
