...
| M2: geometric operator kernel | the CZ24, CCD17, CCY23, CCG25, CCF25, and WBK26 operator identities and constructions are proved at their exact source scope | active; CCG25 now constructs both the tangent-Hessian trace and mean-curvature first jet from a geodesic frame, while realization of that local frame/extension package from neighborhood geometry remains open |
...
The reusable CCG25 differential core is now complete at the pointwise geometric level and connected to the repository's constructed operators. Starting from one isometric immersion, one ambient Levi--Civita connection, and one compatible extension operator, Lean constructs the adjoint orthogonal splitting, the induced Levi--Civita connection, the kernel-normal second fundamental form, and the full continuous trilinear tensor `∇ᴮ II`. Germ-local restriction locality makes this tensor linear even when the global extension operator itself preserves identities only after restriction to the immersed source. The source-checked Codazzi convention `(R̃(W,X)Y)⊥ = (∇ᴮ_W II)(X,Y) - (∇ᴮ_X II)(W,Y)` is then proved and contracted, rather than assumed. A new smooth-extension layer proves that the field-level `∇ᴮ II` on arbitrary `C²` tangent fields depends only on their three values: Codazzi, symmetry, and ambient curvature tensoriality eliminate their first jets. The source-level CCG25 carrier stores one actual tangent field and a genuine `C²` tangent frame geodesic at the base point. Lean traces `II` in that frame to construct the mean-curvature normal field, proves its ambient extension is differentiable and normal-valued, identifies its value with the normalized pointwise trace, and proves its normal derivative is the traced `∇ᴮ II`. Thus the mean-curvature first-jet equality is no longer an input to the strongest paper-facing theorem. The analytic adapter constructs the intrinsic covariant Hessian and the ambient tangent/normal Hessian traces; canonical normal extensions supply the acceleration correction. The intrinsic jet value is the immersion differential of `roughLaplacianAt`; an adapted-basis theorem splits the ambient `roughLaplacianAt`; and both halves are identified. The strongest paper-facing Bochner and Hodge theorems therefore accept neither a mean-curvature jet, tangent-Hessian identity, Laplacian identity, nor Ricci-symmetry premise. CCY23 equation (4.6) remains a separate construction with a proved raywise smooth coefficient, weighted ODE, and exact divergence cancellation. None of the remaining fragments is promoted to exact source reproduction until the smooth local extension operator and geodesic frame are themselves realized from source-level neighborhood geometry. A global tubular retraction is one possible implementation, not a premise of CCG25 Theorem 1.1, which only assumes extensions on a neighborhood of the evaluation point.
...
| `CCG25-bochner-laplacian-gauss-general-codimension` | the intrinsic and ambient values are the constructed rough Laplacians; the adapted trace split and tangent Hessian are proved; a geodesic `C²` tangent frame now constructs the mean-curvature field and its first normal derivative | construct the geodesic frame and smooth compatible extension operator from source-level neighborhood geometry, rather than accepting their regularity package |
| `CCG25-hodge-laplacian-gauss-general-codimension` | scalar Gauss, adapted Ricci splitting, and both Hodge formulas are proved on the same actual fibers; the tangent Hessian, Ricci symmetry, and mean-curvature first jet are constructed internally from the shared geodesic-frame input | discharge the shared neighborhood-realization gate from the Bochner node |
