...
| M2: geometric operator kernel | the CZ24, CCD17, CCY23, CCG25, CCF25, and WBK26 operator identities and constructions are proved at their exact source scope | active; the CCG25 geometric identities now reach actual immersion fibers, while their common tubular field realization remains open |
...
The reusable CCG25 differential core is now complete at the pointwise geometric level. Starting from one isometric immersion, one ambient Levi--Civita connection, and one compatible extension operator, Lean constructs the adjoint orthogonal splitting, the induced Levi--Civita connection, the kernel-normal second fundamental form, and the full continuous trilinear tensor `∇ᴮ II`. Germ-local restriction locality makes this tensor linear even when the global extension operator itself preserves identities only after restriction to the immersed source. The source-checked Codazzi convention `(R̃(W,X)Y)⊥ = (∇ᴮ_W II)(X,Y) - (∇ᴮ_X II)(W,Y)` is then proved and contracted, rather than assumed. Torsion-freeness and the constructed Weingarten formula likewise prove the mean-bracket identity from an explicit first-order realization of the mean-curvature normal field. These results feed a single actual-fiber differentiated jet and then a single Ricci/Hodge jet; equations (1.5)--(1.6) and both Corollary 1.20 formulas follow with scalar Gauss and the adapted ambient Ricci split discharged internally. The paper namespace now exposes these stronger actual-immersion theorems. CCY23 equation (4.6) remains a separate construction with a proved raywise smooth coefficient, weighted ODE, and exact divergence cancellation. None of these results is promoted to an exact source reproduction until the visible analytic realization inputs are constructed from the paper's one smooth tubular neighborhood field.
...
| `CCG25-bochner-laplacian-gauss-general-codimension` | the actual-fiber `∇ᴮ II` tensor, uncontracted and contracted Codazzi, mean-bracket Weingarten conversion, and both Bochner formulas are derived from one immersion/connection package | construct the field-specific intrinsic two-jet, both normal-direction ambient second-jet terms, the mean-curvature first jet, and extension regularity from one smooth tubular neighborhood extension |
| `CCG25-hodge-laplacian-gauss-general-codimension` | the completed differentiated jet is joined to the connection-derived Ricci jet; scalar Gauss, adapted Ricci splitting, and both Hodge formulas are proved on the same actual fibers | discharge the same tubular field-realization gate and identify the resulting pointwise Hodge values with the paper's smooth ambient and intrinsic Laplacians |
...
The critical path is therefore no longer formula discovery, tensoriality of `∇ᴮ II`, Codazzi, bracket--Weingarten, vector/scalar Gauss, or ambient Ricci trace splitting. It is one common smooth-neighborhood-field adapter. For CCG25 that adapter must realize the differentiated-extension regularity contract, the intrinsic field two-jet, the normal-direction ambient second derivatives, and the mean-curvature normal-field first jet from a tubular retraction/transport construction. For CCY23 it must upgrade the proved raywise radial coefficient to a jointly smooth angular-radial field and extract the coordinate jet already consumed by the ellipsoid theorem. Once this adapter is stable, the CCG25 source signatures can be inhabited without analytic jet inputs, the CCY23 restriction and expansion can be connected to the same actual extension, and the remaining WBK26 flow-rate adapter is bounded cleanup. Only then do the six remaining fragments qualify for promotion and M2 close.
