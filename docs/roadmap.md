...
| M2: geometric operator kernel | the CZ24, CCD17, CCY23, CCG25, CCF25, and WBK26 operator identities and constructions are proved at their exact source scope | active; CCG25 now constructs its geodesic frame, tangent-Hessian trace, and mean-curvature first jet internally, leaving the smooth ambient-extension realization as its shared open gate |
...
| `CCG25-bochner-laplacian-gauss-general-codimension` | the intrinsic and ambient values are the constructed rough Laplacians; the adapted trace split, tangent Hessian, explicit geodesic frame, mean-curvature field, and its first normal derivative are all constructed from an ordinary `C²` field and pointwise bases | construct the smooth compatible ambient-extension operator and its regularity contract from source-level neighborhood geometry |
| `CCG25-hodge-laplacian-gauss-general-codimension` | scalar Gauss, adapted Ricci splitting, and both Hodge formulas are proved on the same actual fibers; the geodesic frame, tangent Hessian, Ricci symmetry, and mean-curvature first jet are constructed internally | discharge the shared smooth ambient-extension gate from the Bochner node |
...
The critical path is therefore no longer formula discovery, construction of a geodesic tangent frame, pointwise tensoriality of `∇ᴮ II`, Codazzi, bracket--Weingarten, the mean-curvature first-jet calculation, vector/scalar Gauss, ambient Ricci splitting, construction of the source field's intrinsic and ambient Hessians, either adapted-frame Hessian trace, Ricci symmetry, a generic operator-identification problem, or the material-trajectory chain rule. It is one reusable geometric obligation: construct the smooth local ambient extensions used by the source calculations and derive their regularity from neighborhood data. For CCG25 it must supply compatible tangent/normal extensions, their smoothness-preservation contract, and the concrete Euclidean flatness specialization used by the codimension-two corollary. For CCY23 it must supply joint angular-radial smoothness and the coordinate jet already consumed by the ellipsoid result. A tubular retraction may implement these fields, but the public theorem must follow the source's weaker local-neighborhood hypothesis. Once this package is stable, the CCG25 source signatures can be inhabited without caller-supplied intermediate geometric facts and the CCY23 restriction and expansion can use the same neighborhood mechanism. The remaining WBK26 step is then to construct the connecting fields as pushforwards by a generated local flow. Only then do the six remaining fragments qualify for promotion and M2 close.
