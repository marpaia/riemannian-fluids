...
| M2: geometric operator kernel | the CZ24, CCD17, CCY23, CCG25, CCF25, and WBK26 operator identities and constructions are proved at their exact source scope | active; CCG25 now has constructed normal-form-chart extensions, a chart-independent symmetric second fundamental form, and a locally induced Levi--Civita/Gauss bridge, leaving the differentiated germ-local adapter and Euclidean specialization as its shared open gate |
...
The reusable CCG25 differential core is now complete at the pointwise geometric level and connected to the repository's constructed operators. Starting from one isometric immersion, one ambient Levi--Civita connection, and one compatible extension operator, Lean constructs the adjoint orthogonal splitting, the induced Levi--Civita connection, the kernel-normal second fundamental form, and the full continuous trilinear tensor `∇ᴮ II`. Germ-local restriction locality makes this tensor linear even when the global extension operator itself preserves identities only after restriction to the immersed source. The source-checked Codazzi convention `(R̃(W,X)Y)⊥ = (∇ᴮ_W II)(X,Y) - (∇ᴮ_X II)(W,Y)` is then proved and contracted, rather than assumed. A smooth-extension layer proves that the field-level `∇ᴮ II` on arbitrary `C²` tangent fields depends only on their three values: Codazzi, symmetry, and ambient curvature tensoriality eliminate their first jets. A general connection theorem now constructs the required geodesic frame rather than storing it: the canonical tangent-bundle trivialization is corrected by chart-affine scalar functions whose prescribed differentials cancel every connection coefficient at the base point. Starting only from an ordinary `C²` source field and pointwise orthonormal bases, Lean obtains a `C²` geodesic frame, traces `II` to construct the mean-curvature normal field, proves its ambient extension is differentiable and normal-valued, identifies its value with the normalized pointwise trace, and proves its normal derivative is the traced `∇ᴮ II`. The analytic adapter constructs the intrinsic covariant Hessian and the ambient tangent/normal Hessian traces; canonical normal extensions supply the acceleration correction. The intrinsic jet value is the immersion differential of `roughLaplacianAt`; an adapted-basis theorem splits the ambient `roughLaplacianAt`; and both halves are identified. The strongest paper-facing Bochner and Hodge theorems therefore accept neither a geodesic-frame field, mean-curvature jet, tangent-Hessian identity, Laplacian identity, nor Ricci-symmetry premise. The previously open local-extension construction is now real: `SmoothIsometricEmbeddingData` records Mathlib's immersion normal form, `immersionChartRetractionAt` constructs the chart projection, and `LocalSubmanifoldExtensionDataAt` constructs compatible scalar, tangent, and along-field extensions with germ agreement and both differentiability and arbitrary finite-order regularity preservation. Those local extensions now construct a chart-independent symmetric continuous bilinear second fundamental form in the actual kernel-normal fiber. A family of point-centered packages also assembles into a genuine induced covariant derivative; ambient metric compatibility and torsion-freeness make it Levi--Civita, uniqueness identifies it with any intrinsic source Levi--Civita connection on differentiable germs, and the local Gauss formula follows with the actual intrinsic derivative. The older second-order CCG engine is still parameterized by its global extension package, so the paper nodes remain fragments until its curvature, `∇ᴮ II`, mean-curvature, and Hessian routes are migrated to the new germ-local API. CCY23 equation (4.6) remains a separate construction with a proved raywise smooth coefficient, weighted ODE, and exact divergence cancellation.
...
| `CCG25-gauss-ricci-codimension-two` | differentiated Gauss--Weingarten, field/pointwise shape agreement, vector Gauss, scalar Gauss, and the flat-ambient `Fin 2` Ricci contraction are derived from one actual-fiber immersion package; local chart extensions now construct symmetric `II`, the induced Levi--Civita connection, and intrinsic Gauss | migrate Codazzi and the curvature trace to the germ-local package and construct ambient Euclidean flatness |
| `CCG25-bochner-laplacian-gauss-general-codimension` | the intrinsic and ambient values are the constructed rough Laplacians; the adapted trace split, tangent Hessian, explicit geodesic frame, mean-curvature field, and its first normal derivative are all constructed; the needed local extensions now exist with `C²` preservation | route the second-order field, mean-curvature, and Hessian constructions through the germ-local extension API |
| `CCG25-hodge-laplacian-gauss-general-codimension` | scalar Gauss, adapted Ricci splitting, and both Hodge formulas are proved on the same actual fibers; the geodesic frame, tangent Hessian, Ricci symmetry, and mean-curvature first jet are constructed internally | discharge the shared second-order germ-local adapter from the Bochner node |
...
| `WBK26-lie-strain` | an actual particle integral curve differentiates the metric pairing, and a Lie-dragged connecting-pair jet has material rate `L_u g = 2 Def u` | construct the connecting fields as spatial pushforwards by the generated local flow, rather than accepting their first-order Lie-drag equations |

The critical path is therefore no longer formula discovery, construction of a geodesic tangent frame, pointwise tensoriality of `∇ᴮ II`, Codazzi, bracket--Weingarten, the mean-curvature first-jet calculation, vector/scalar Gauss, ambient Ricci splitting, construction of the source field's intrinsic and ambient Hessians, either adapted-frame Hessian trace, Ricci symmetry, a generic operator-identification problem, the material-trajectory chain rule, existence and regularity of local ambient extensions, construction of local `II`, or identification of the locally induced connection with the intrinsic Levi--Civita connection. It is now an integration obligation: differentiate the fixed-chart Gauss germ, package local `∇ᴮ II`, and route Codazzi, curvature traces, mean curvature, and Hessians through it before instantiating the paper theorems from `SmoothIsometricEmbeddingData`. The codimension-two node additionally needs the concrete Euclidean Levi--Civita curvature tensor to vanish. CCY23 must then reuse the same neighborhood mechanism to prove joint angular-radial smoothness and extract the coordinate jet already consumed by the ellipsoid result. The remaining WBK26 step is to construct the connecting fields as pushforwards by a generated local flow. Only then do the six remaining fragments qualify for promotion and M2 close.
