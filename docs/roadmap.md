...
| M2: geometric operator kernel | the CZ24, CCD17, CCY23, CCG25, CCF25, and WBK26 operator identities and constructions are proved at their exact source scope | active; CCG25 now has constructed normal-form-chart extensions and chart-independent first-order projected derivatives, leaving the second-order germ-local adapter and Euclidean specialization as its shared open gate |
...
| `CCG25-gauss-ricci-codimension-two` | differentiated Gauss--Weingarten, field/pointwise shape agreement, vector Gauss, scalar Gauss, and the flat-ambient `Fin 2` Ricci contraction are derived from one actual-fiber immersion package; local chart extensions and chart-independent first derivatives are now constructed | migrate the differentiated Gauss route to the germ-local package and construct ambient Euclidean flatness |
