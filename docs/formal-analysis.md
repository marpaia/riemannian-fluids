...
| smooth local-field realization of the remaining CCG25 data | normal-form chart retraction, compatible scalar/tangent/along extensions, germ agreement, arbitrary finite-order regularity preservation, and chart-independent first projected derivatives are constructed; open: migrate the second-order CCG curvature, mean-curvature, and Hessian engine from its older global-extension package to this local API |
