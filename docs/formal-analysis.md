...
| smooth local-field realization of the remaining CCG25 data | normal-form chart retraction, compatible scalar/tangent/along extensions, germ agreement, finite-order regularity preservation, chart-independent symmetric `II`, a locally induced Levi--Civita connection, intrinsic Gauss, and a symmetric continuous trilinear local `∇ᴮ II` are constructed; open: prove local Codazzi and migrate curvature, mean-curvature, and Hessian routes from the older global package |
