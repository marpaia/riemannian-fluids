...
| `CCG25-gauss-ricci-codimension-two` | proved fragment | immersion normal form → constructed germ-local extensions → symmetric chart-independent `II` → locally induced Levi--Civita connection → intrinsic Gauss; separately, canonical splitting → differentiated Gauss--Weingarten → vector/scalar Gauss → flat-ambient `Fin 2` Ricci contraction | differentiate the local Gauss germ, migrate Codazzi/curvature traces, and construct the Euclidean flat ambient tensor |
| `CCG25-laplacian-gauss-family` | split: two proved fragments | constructed germ-local `C²` extensions, `II`, and intrinsic Gauss; separately, actual `C²` fields → intrinsic rough operator + adapted ambient rough trace → tangent/normal Hessian traces → Bochner formulas → Ricci symmetry → Hodge operators | route local `∇ᴮ II`, curvature, mean-curvature, and Hessian fields through the germ-local package |
