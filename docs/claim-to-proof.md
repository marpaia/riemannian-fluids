...
| `CCG25-gauss-ricci-codimension-two` | proved fragment | immersion normal form → constructed germ-local extensions → chart-independent first projected derivatives; separately, canonical splitting → differentiated Gauss--Weingarten → vector/scalar Gauss → flat-ambient `Fin 2` Ricci contraction | migrate differentiated Gauss to the local package and construct the Euclidean flat ambient tensor |
| `CCG25-laplacian-gauss-family` | split: two proved fragments | constructed germ-local `C²` extensions; separately, actual `C²` fields → intrinsic rough operator + adapted ambient rough trace → tangent/normal Hessian traces → Bochner formulas → Ricci symmetry → Hodge operators | route the second-order curvature, mean-curvature, and Hessian engine through the germ-local package |
