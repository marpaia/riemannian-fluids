...
| `CCG25-laplacian-gauss-family` | split: two proved fragments | actual-fiber `∇ᴮ II` → proved Codazzi contraction and bracket--Weingarten → single-source Bochner jet → connection Ricci jet → actual-fiber Bochner and Hodge formulas | realize every remaining field jet and regularity hypothesis from one smooth tubular neighborhood extension, then identify the pointwise values with the source Laplacians |
