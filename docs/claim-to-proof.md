...
| `CCG25-laplacian-gauss-family` | split: two proved fragments | actual `C²` source/ambient fields → constructed intrinsic rough operator + adapted ambient rough trace → proved normal Hessian trace → explicit tangent-Hessian bridge → actual-fiber Bochner formulas → first Bianchi/metric-skew Ricci symmetry → constructed Hodge operators | construct the tubular extension package and mean-curvature first jet, then derive the varying-field tangent-Hessian bridge |
