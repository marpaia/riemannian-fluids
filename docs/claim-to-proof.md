...
| `CCG25-gauss-ricci-codimension-two` | proved fragment | canonical isometric splitting → induced Levi--Civita connection → differentiated Gauss--Weingarten → Riesz/field shape agreement → vector and scalar Gauss → flat-ambient `Fin 2` Ricci contraction | realize the regularity contract and flat ambient tensor from a smooth tubular extension in concrete Euclidean space |
| `CCG25-laplacian-gauss-family` | split: two proved fragments | actual `C²` source/ambient fields → constructed intrinsic rough operator + adapted ambient rough trace → proved normal Hessian trace → explicit tangent-Hessian bridge → actual-fiber Bochner formulas → Ricci jet and constructed Hodge operators | construct the tubular extension package and mean-curvature first jet, derive the varying-field tangent-Hessian bridge, and prove Levi--Civita Ricci symmetry |
