...
| `CCG25-laplacian-gauss-family` | split: two proved fragments | actual `C²` source/ambient fields → constructed intrinsic rough operator + adapted ambient rough trace → proved normal and tangent Hessian traces → actual-fiber Bochner formulas → first Bianchi/metric-skew Ricci symmetry → constructed Hodge operators | construct the smooth local-extension package, its differentiated regularity, and the mean-curvature first jet from source-level neighborhood data |
