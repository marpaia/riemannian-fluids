...
| `CCG25-gauss-ricci-codimension-two` | proved fragment | canonical isometric splitting → proved related-bracket naturality and Levi--Civita restriction locality → induced Levi--Civita connection → same-source `II` and projected ambient curvature → symmetry → vector-to-scalar Gauss contraction | differentiate Gauss--Weingarten under the exact second-order regularity contract and derive vector Gauss |
