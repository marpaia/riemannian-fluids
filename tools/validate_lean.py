...
def audit_crosswalk_axioms(declarations: list[tuple[str, str, str]]) -> None:
    ...
    for claim_id, _, _ in declarations:
        ...
        if "sorryAx" in segment:
            failures.append(f"{claim_id} reaches sorryAx")
    if failures:
...
def main() -> None:
    ...
    declarations = crosswalk_declarations()
    audit_crosswalk_axioms(declarations)
    print(
        f"validated thematic Lean closure: {len(library)} files, "
        f"0 placeholders, {len(declarations)} selective literature crosswalks"
    )
